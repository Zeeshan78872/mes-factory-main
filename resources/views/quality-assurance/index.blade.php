        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="card-header">
                  <h4 class="card-title">
                    All QA Inspections <a href="{{ route('production.daily') }}" class="btn btn-sm btn-primary float-end"><i class="fa fa-plus"></i> Add New</a>
                  </h4>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <table class="table table-striped dt-responsive nowrap" id="dataTable" style="width:100%">
                    ...
                    <tr>
                      ...
                      <th class="align-middle">QA Type</th>
                      <th class="align-middle">Category</th>
                      <th class="align-middle">Department</th>
                      <th class="align-middle">Stock Card</th>
                      <th class="align-middle">Inpection By</th>
                      <th class="align-middle">Inpection Date</th>
                      <th class="align-middle">Total Defects Found <br>Cr | Mj | Mn</th>
                      <th class="align-middle">Total Defects Allowed <br>Cr | Mj | Mn</th>
                      <th class="align-middle">Actions</th>
                    </tr>
                    </thead>
                    <tbody>
                      
                      @foreach ($QASurveys as $QASurvey)
                      <tr>
                        <td><a href="{{ route('quality-assurance.show', $QASurvey->id) }}">{{ $QASurvey->id }}</a></td>
                        <td>{{ $QATypes[$QASurvey->qa_type] }}</td>
                        <td>{{ $QACategories[$QASurvey->qa_category ?? 0] }}</td>
                        <td>{{ $QASurvey->department->name ?? 'N/A'}}</td>
                        <td>{{ $QASurvey->stockCard->stock_card_number}}</td>
                        <td><a href="{{ route('users.show', $QASurvey->qa_by) }}" target="_blank">{{ $QASurvey->inspectionBy->name }}</a></td>
                        <td>{{ $QASurvey->created_at }}</td>
                        <td>
                          {{ $QASurvey->total_defects_found_cr }} | {{ $QASurvey->total_defects_found_mj }} | {{ $QASurvey->total_defects_found_mn }}
                        </td>
                        <td>
                          {{ $QASurvey->total_defects_allowed_cr }} | {{ $QASurvey->total_defects_allowed_mj }} | {{ $QASurvey->total_defects_allowed_mn }}
                        </td>
                        <td>
                          <a class="btn btn-primary btn-sm" href="{{ route('quality-assurance.show', $QASurvey->id) }}">View</a>
                        </td>
                      </tr>
                      @endforeach
...
<script>
  $(document).ready(function() {
    ...
    var exportTitle = 'quality-assurance-surveys-data-export-' + Date.now();
